...
{
    // A directed activity edge: a straight line clipped to the source/target event boxes with an
    // arrowhead at the target, plus the activity label placed at the edge midpoint. Endpoints and
    // label position are derived from the node positions and update live as nodes are dragged.
    // Unlike the vertex edge, the base colour is the activity colour (so the critical path shows
    // through); selection still overrides it with the highlight colour.
    public class ArrowGraphEdgeViewModel
        : ReactiveObject, IDisposable
    {
        ...
        private readonly ArrowGraphNodeViewModel m_Source;
        private readonly ArrowGraphNodeViewModel m_Target;
        ...
        public ArrowGraphEdgeViewModel(
            int id,
            ArrowGraphNodeViewModel source,
            ArrowGraphNodeViewModel target,
        ...
        private static Point ClipToBorder(ArrowGraphNodeViewModel node, ArrowGraphNodeViewModel toward)
        {
