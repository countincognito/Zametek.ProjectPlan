...
{
    // The reusable, self-contained interactive vertex graph. It builds the graph, runs the MSAGL
    // layout, populates the draggable/selectable node and edge view-models the InteractiveVertexGraphView
    ...
    // domain graph, the save dialog and error reporting) is supplied through IVertexGraphHost, so a
    // consumer can drop the control in without writing any of this machinery. (Parallel to
    // InteractiveArrowGraphViewModel, minus ShowNames and single-/multi-line edge labels.)
    public class InteractiveVertexGraphViewModel
        : ReactiveObject, IInteractiveVertexGraph, IDisposable
    {
        #region Fields

        private readonly IVertexGraphHost m_Host;
        private readonly IVertexGraphSerializer m_Serializer;
        ...
        public InteractiveVertexGraphViewModel(
            IVertexGraphHost host,
            IVertexGraphSerializer serializer,
            IGraphImageExporter imageExporter)
        {
            ...
            SaveVertexGraphImageFileCommand = ReactiveCommand.CreateFromTask(SaveInteractiveImageAsync);
        ...
        // Interactive vertex-graph bindings consumed by InteractiveVertexGraphView.
        ...
        // thread. Re-raises Theme so the background stays in step.
        public void Refresh()
        {
            try
            {
        ...
        public async Task SaveImageAsync(string? filename, VertexGraphImageSource source)
        {
            ...
            try
            {
                ...
                fileExtension.ValueSwitchOn()
                    ...
                    {
                        data = m_Serializer.BuildVertexGraphMLData(m_Host.BuildDiagram());
                    })
                    .Case($".{GraphFileExtensions.GraphViz}", _ =>
                    {
                        data = m_Serializer.BuildVertexGraphVizData(m_Host.BuildDiagram());
                    })
        ...
        private GraphLayoutModel BuildLayout()
        {
            return m_Host.HasCompilationErrors
                ...
                : m_Serializer.BuildVertexGraphLayout(m_Host.BuildDiagram(), m_Host.Theme);
        }
        ...
        private void PopulateInteractiveGraph(GraphLayoutModel layout)
        {
            ...
            foreach (GraphEdgeLayoutModel edgeLayout in layout.Edges)
            {
                ...
                GraphEdges.Add(new VertexGraphEdgeViewModel(
                    edgeLayout.Id,
                    source,
                    target,
        ...
        private async Task SaveInteractiveImageAsync()
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(filename))
                {
                    await SaveImageAsync(filename, VertexGraphImageSource.InteractiveCanvas);
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task SaveImageFormatAsync(string filename, string fileExtension, VertexGraphImageSource source)
        {
            if (source == VertexGraphImageSource.InteractiveCanvas)
            {
            ...
            byte[] svgData = m_Serializer.BuildVertexGraphSvgData(m_Host.BuildDiagram(), m_Host.Theme);
