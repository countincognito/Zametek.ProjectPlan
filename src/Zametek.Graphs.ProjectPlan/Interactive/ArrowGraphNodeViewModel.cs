...
{
    // Interactive, draggable, selectable arrow-graph event node. Mirrors VertexGraphNodeViewModel:
    // the event box shows its earliest/latest-finish-time label, the border colour/dash is
    // preserved, selection is shown via a separate overlay ring (in the view) and dimming is driven
    // by opacity. (Kept as a parallel type to the vertex node so the two interactive graphs can
    // evolve independently.)
    public class ArrowGraphNodeViewModel
        : ReactiveObject
    {
        private const double c_DimmedOpacity = 0.25;

        public ArrowGraphNodeViewModel(GraphNodeLayoutModel layout)
        {
