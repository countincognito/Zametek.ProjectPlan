...
{
    // Reusable, embeddable interactive arrow-graph viewer. It draws against any host that
    // implements IInteractiveArrowGraph (e.g. the application's ArrowGraphManagerViewModel),
    // supporting drag, click-to-select, hover tooltips, unbounded pan and zoom. (Parallel to
    // InteractiveVertexGraphView.)
    public partial class InteractiveArrowGraphView
        : UserControl
    {
        ...
        public InteractiveArrowGraphView()
        {
            InitializeComponent();
        ...
        private void Viewport_PointerPressed(object? sender, PointerPressedEventArgs e)
        {
            ...
            (DataContext as IInteractiveArrowGraph)?.SelectNode(null);
        ...
        {
            if (DataContext is not IInteractiveArrowGraph viewModel
            ...
            foreach (ArrowGraphNodeViewModel node in viewModel.GraphNodes)
            {
