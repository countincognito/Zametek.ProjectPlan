...
{
    // The edge-routing strategy. These map by name to Microsoft.Msagl.Core.Routing.EdgeRoutingMode
    // (inside the serializer, so the library's own configuration surface carries no MSAGL types), and
    // they also drive the interactive view's client-side edge shape (see
