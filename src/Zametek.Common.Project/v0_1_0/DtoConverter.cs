...
namespace Zametek.Common.Project.v0_1_0
{
    public static class DtoConverter
    {
        ...
        public static GraphCompilation<int, IDependentActivity<int>> FromDto(GraphCompilationDto dto)
        {
            ...
            bool errorsExist = dto.AllResourcesExplicitTargetsButNotAllActivitiesTargeted
                || dto.CircularDependencies.Any()
                || dto.MissingDependencies.Any();
            ...
            int duration)
        {
            ...
            {
                AllResourcesExplicitTargetsButNotAllActivitiesTargeted = graphCompilation?.Errors?.AllResourcesExplicitTargetsButNotAllActivitiesTargeted != null ? graphCompilation.Errors.AllResourcesExplicitTargetsButNotAllActivitiesTargeted : false,
                CircularDependencies = graphCompilation?.Errors?.CircularDependencies != null ? graphCompilation.Errors.CircularDependencies.Select(x => ToDto(x)).ToList() : new List<CircularDependencyDto>(),
                MissingDependencies = graphCompilation?.Errors?.MissingDependencies != null ? graphCompilation.Errors.MissingDependencies.ToList() : new List<int>(),
